...
export default new class Orientation extends CustomEvent {
	...
	init( $window = $( window ), $body = $( document.body ), xAmp = this.X_AMP, yAmp = this.Y_AMP, needDispatchEvent = false, debug = this.DEBUG ) {
		this._initDOM( $window, $body );
	...
	_bindEvents() {
		this.eh.bodyTouch = this._askOrientation.bind( this );
		this.$body.on( 'touchstart', this.eh.bodyTouch );
	...
	_askOrientation() {
		this.$body.off( 'touchstart', this.eh.bodyTouch );
	...
	_onOrientationChange() {
		if ( Device.IS_DESKTOP || this.prevbW == Screen.bW )
			return;
